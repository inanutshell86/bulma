...
class PhotosController extends Controller
{
    ...
    public function index()
    {
        ...
        $photos = $this->db->getPaginatedFrom('photos', 'user_id', $this->auth->getUserId(), $page, $perPage);

        $paginator = paginate(
            $this->db->getCount('photos', 'user_id', $this->auth->getUserId()),
            $page,
            $perPage,
            "/photos?page=(:num)"
        );
    ...
    {
        $photo = $this->db->find('photos', $id);
        $user = $this->db->find('users', $photo['user_id']);
        $userImages = $this->db->whereAll('photos', 'user_id', $user['id'], 4);
